...
\stitle{Visual quality} Figure~\ref{fig:rate_quality} reports the visualization quality (defined in Equation~\eqref{eqref:loss}) of the trajectory sampling methods under different sampling rates.
...
% and the visualization time of $\full$ (which does not change) is included at the top of each figure for reference.
...
\stitle{Optimizations and costs} In Figure~\ref{fig:rate_algtime}, we report the running time of $\vatss$  with and without the heap-based lazy computation.
